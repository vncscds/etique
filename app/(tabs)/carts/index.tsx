import Cart from "@/modules/cart/@types/Cart.type";
      ...
      <View style={{ borderWidth: 1, padding: 16, borderRadius: 7, borderColor: Colors.MAIN_COLORS.ETIQUE.C2, gap: 10 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <View>
            <View style={{ minWidth: '100%', justifyContent: 'space-between', flexDirection: 'row' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                  <Typography fontFamily="Inter_700Bold" fontSize="lg" fontColor={Colors.MAIN_COLORS.ETIQUE.C2}>{cart.title}</Typography>
                  <Typography fontSize="xs" fontColor={Colors.NEUTRAL_COLORS.C6}>({cart.products.length} itens)</Typography>
                </View>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                <Badge
                  icon={cart.isActive ? CheckCircle2 : XCircleIcon}
                  title={cart.isActive ? 'Ativo' : 'Inativo'}
                />
                <Pressable style={({ pressed }) => ({ opacity: pressed ? .5 : 1 })} onPress={() => {
                  toast.show('Botão pressionado...', {
                    description: 'Abrir bottom sheet de opções...'
                  });
                }}>
            ...
            <Typography fontColor={Colors.MAIN_COLORS.ETIQUE.C2} fontSize="xs">{cart.description}</Typography>
            <Typography fontSize="2xs" fontColor={Colors.NEUTRAL_COLORS.C4}>ID: {cart.id}</Typography>
          </View>
        </View>
        <View style={{ width: '100%', padding: .5, backgroundColor: Colors.MAIN_COLORS.ETIQUE.C1 }} />
        <View>
          <View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}>
            ...
            <View style={{ flexDirection: 'row', gap: 4 }}>
              ...
              <Typography fontSize="xs" fontColor={Colors.MAIN_COLORS.ETIQUE.C2}>{dayjs(cart.createdAt).format('DD/MM/YYYY HH:mm')}</Typography>
            </View>
          </View>
          <View style={{ flexDirection: 'row', gap: 4, alignItems: 'center' }}>
        ...
        <View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Typography fontSize="2xs" fontColor={Colors.NEUTRAL_COLORS.C6}>Subtotal</Typography>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Typography fontSize="2xs" fontColor={Colors.NEUTRAL_COLORS.C6}>Descontos</Typography>
            <Typography fontSize="2xs" fontColor={Colors.NEUTRAL_COLORS.C6} customStyle={{ textDecorationLine: 'line-through' }}>{
              new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(+cart.total)
            }</Typography>
          </View>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Typography fontSize="sm" fontFamily="Inter_700Bold" fontColor={Colors.MAIN_COLORS.ETIQUE.C1}>Total</Typography>
    ...
    <React.Fragment>
      <Container customStyle={{
        ...
        <List
          ...
          data={carts.data}
          ListEmptyComponent={
            carts.isLoading ? <Typography>Carregando...</Typography> : <EmptyCarts
              onPressNewCart={() => {
      ...
      <BottomSheet ref={bottomSheetRef}>
        <BottomSheetScrollView contentContainerStyle={{ padding: 16, gap: 16, paddingBottom: 36 }} keyboardShouldPersistTaps="handled">
          <View>
            <Typography fontFamily="Inter_700Bold" fontSize="lg">Novo carrinho</Typography>
            <Typography fontColor={Colors.NEUTRAL_COLORS.C6} fontSize="xs">Criação de um novo carrinho</Typography>
          </View>
          <View>
            <Typography>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quo, suscipit, doloremque beatae et excepturi dolore accusamus numquam similique impedit assumenda, temporibus nostrum quis odit! Dolore ab facilis consectetur enim eligendi! Esse, qui?</Typography>
